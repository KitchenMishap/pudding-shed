...
func TestGenesisBlock(t *testing.T) {
	result := Block{}
	err := ParseJsonBlock([]byte(genesisBlock), &result)
	if err != nil {
		t.Error(err)
	}
	if result.Version != 1 {
		t.Error("Expected block version to be 1")
	}
	if result.Transactions[0].Version != 1 {
		t.Error("Expected transaction version to be 1")
	}
}
