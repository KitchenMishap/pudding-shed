...
Tier 0 is the top tier of the wedding cake, representing the most recent and possibly most frequently accessed data.
As time progresses, older data is moved to lower tiers (1+), allowing for efficient retrieval and storage.
We build the tiered cake from the top down.
### Tier 0 is different
Tier 0 is different, being primarily implemented as an in-memory map. Hashes are only ever presented to tier 0.
### Tiers 1+ comprised of up to 255 DonutForest's
Each tier 1+ is comprised of zero or more concentric DonutForest's (they are Donuts from the high level
...
* Tier 0 is "full" when it has been presented with 65535 hashes (including those that were duplicates of prior hashes). 
    * All hashes are baked into a single DonutForest in tier 1.
    * Tier 0 then becomes empty, ready for more hash injections
* Tier 1 is "full" when it contains 255 DonutForests.
    * Hashes from ALL 255 of tier 1's DonutForests are baked into a single DonutForest in tier 2.
    * Consequently, DonutForest's are much bigger in tier 2 compared to tier 1.
    * Tier 1 then becomes empty.
* Similarly, tiers 2+ are "full" when they contain 255 DonutForests.
