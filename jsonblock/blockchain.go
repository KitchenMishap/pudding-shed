...
func PostJsonEncodeAddressHashes(block *JsonBlockEssential) error {
	for nthTrans := range block.J_tx {
		...
		for nthTxo := range transPtr.J_vout {
			...
			adornTxoAddressWithPuddingHash3(addrPtr)
		}
	}
	return nil
}

func adornTxoAddressWithPuddingHash3(addrPtr *JsonScriptPubKeyEssential) {
	hexval := strings.ToLower(addrPtr.J_hex)
	binval, _ := hex.DecodeString(hexval)

	// THE FOLLOWING HASH "PuddingHash3" IS PECULIAR TO PUDDING SHED SOFTWARE AND NOT IN GENERAL USE BY BITCOINERS
	// BUT NOTE THAT IT CAN BE GENERATED BY HASHING THE SCRIPT THAT A "USUAL" ASCII ADDRESS STRING PRODUCES
	// The hash we use to identify an address is as follows: (this hash is peculiar to pudding-shed)
	// It is the hash of the raw-bytes (not ascii hex) in the script pub key
	hashval := indexedhashes.HashOfBytes(binval)

	addrPtr.puddingHash3 = hashval
}
