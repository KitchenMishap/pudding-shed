...
func TestConcreteHashStoreCreator(t *testing.T) {
	...
	HelperTestConcreteHashStoreCreator(t, 20, 3, 2, 100000)
	println("Finished short intensive test (1 of 3)")

	// Test with parameters representative of block hashes
	HelperTestConcreteHashStoreCreator(t, 30, 3, 3, 30000)
	...
	HelperTestConcreteHashStoreCreator(t, 30, 4, 3, 20000)
...
func HelperTestConcreteHashStoreCreator(t *testing.T, phbc int64, ebc int64, cpc int64, iters uint64) {
	var abstractCreator HashStoreCreator

	// Create the creator
	abstractCreator, err := NewConcreteHashStoreCreator("Test", "Temp_Testing", phbc, ebc, cpc)
	if err != nil {
